...
void SpaceInvaders::HighscoreImpl::begin(){
...
void SpaceInvaders::HighscoreImpl::add(const Score& score){
...
void SpaceInvaders::HighscoreImpl::clear(){
	data.count = 0;
	save();
}

const SpaceInvaders::Score& SpaceInvaders::HighscoreImpl::get(uint8_t i){
	return data.scores[i];
}

uint8_t SpaceInvaders::HighscoreImpl::count(){
	return data.count;
}
